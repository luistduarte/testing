...
public class Run extends AbstractVerticle {
	...
	private String dataENV = "WALLETS";
	...
	public void start() throws Exception {
		...
		try {
			...
			String envDATA = System.getenv("DATA");
			...
			if (envDATA != null) {
				System.out.println("env DATA" + System.getenv("DATA"));
				dataENV = System.getenv("DATA");
			}
		...
		switch (dataENV) {
			...
			case "CODES":
				...
				break;
			default :
				System.out.println("No INFO to get");
				break;			
		}
			...
			if (asyncResult.succeeded()) {
				...
				transactionsFuture.setHandler(transResult -> {
					if (transResult.succeeded()) {
						...
						System.out
								.println("user_walking_context" + " Total:" + sumValuesWalk + " Size:" + nrDataObjects);
						System.out.println("sumValuesWalk" + " Avg:" + sumValuesWalk / nrDataObjects);

						System.out
								.println("user_biking_context" + " Total:" + sumValuesBike + " Size:" + nrDataObjects);
			...
			if (asyncResult.succeeded()) {
				...
				for (int x = 0; x < allWallets.size(); x++) {
					...
					if (!address.equals("public-wallets")) {
						...
						/*
						 * if (wallet.containsKey("transactions")) { wallet.remove("transactions");
						 * 
						 * mongoClient.findOneAndReplace("wallets", new JsonObject().put("_id",
						 * walletID), wallet, r -> {}); }
						 */
				...
				for (int i = 0; i < shops.size(); i++) {
					...
					bonus.setHandler(bonusResult -> {
						...
						if (bonusResult.succeeded()) {
							System.out.println(
									"[BONUS][" + id + "]  (" + bonus.result().size() + "):" + shop.getString("name"));
						}
	...
	Future<JsonArray> getBonusForShop(String shopID) {
		...
		mongoClient.find("bonus", new JsonObject().put("spotID", shopID), resultHandler -> {
			...
			if (resultHandler.result().size() != 0) {
				...
				mongoClient.find("transactions", new JsonObject().put("source", "bonus").put("data",
						new JsonObject().put("shopID", shopID).put("bonusID", bonusID)), res -> {
	...
	Future<JsonArray> getTransactionsSource(String source) {
		...
		Future<JsonArray> allTransactions = Future.future();
		mongoClient.find("transactions", new JsonObject().put("source", source), res -> {
	...
	Future<Integer> countDataObjects(String ratingType) {
		...
		mongoClient.find("dataobjects", new JsonObject().put("ratingType", ratingType), res -> {
